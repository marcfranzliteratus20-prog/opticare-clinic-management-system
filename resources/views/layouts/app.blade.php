    ...
    @php
        ...
        try {
            if (\Illuminate\Support\Facades\Schema::hasTable('appointments')) {
                $appointmentQuery = \Illuminate\Support\Facades\DB::table('appointments');
                ...
                if (\Illuminate\Support\Facades\Schema::hasColumn('appointments', 'appointment_date')) {
        ...
        try {
            if (\Illuminate\Support\Facades\Schema::hasTable('billings')) {
                ...
                if (\Illuminate\Support\Facades\Schema::hasColumn('billings', 'payment_status')) {
                    $unpaidBilling = (clone $billingQuery)
                        ->whereRaw('LOWER(payment_status) = ?', ['unpaid'])
                        ->count();
                } elseif (\Illuminate\Support\Facades\Schema::hasColumn('billings', 'status')) {
                    $unpaidBilling = (clone $billingQuery)
                        ->whereRaw('LOWER(status) = ?', ['unpaid'])
                        ->count();
                }
            }
        } catch (\Throwable $e) {
        ...
        try {
            if (\Illuminate\Support\Facades\Schema::hasTable('inventories')) {
                ...
                if (\Illuminate\Support\Facades\Schema::hasColumn('inventories', 'stock')) {
                    $lowStock = (clone $inventoryQuery)
                        ->where('stock', '<=', 10)
                        ->count();
                } elseif (\Illuminate\Support\Facades\Schema::hasColumn('inventories', 'quantity')) {
                    $lowStock = (clone $inventoryQuery)
                        ->where('quantity', '<=', 10)
                        ->count();
                }
